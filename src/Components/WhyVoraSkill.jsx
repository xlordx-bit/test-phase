    ...
    <section className="py-28 px-6 bg-gradient-to-tr from-[#1a1a2e] to-[#12122a] text-white">
      ...
      >
        ...
        <p className="mt-6 text-gray-300 text-lg leading-relaxed">
          We make learning simple, fast, and effective through micro-certification and gamified content.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto">
        {features.map((item, index) => (
          <motion.div
            key={index}
            whileHover={{ scale: 1.07, boxShadow: "0 20px 40px rgba(255, 255, 255, 0.12)" }}
            whileTap={{ scale: 0.97 }}
            transition={{ type: "spring", stiffness: 300 }}
            className="bg-gradient-to-br from-[#2e2e5a] to-[#212153] p-8 rounded-3xl shadow-lg flex flex-col items-center text-center cursor-pointer select-none"
          >
            <div className="mb-6 p-4 rounded-full bg-white bg-opacity-10 flex justify-center items-center">
              {item.icon}
            </div>
            <h3 className="text-2xl font-semibold mb-3 drop-shadow-md">{item.title}</h3>
